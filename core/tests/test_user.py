...
from core.models import User, Role, Country
...
def invalid_user_payloads(country):
    ...
    return {
        "password_mismatch": {
            ...
            "role": "COUNTRY_MEMBER", "country_id": country.id
        },
        "weak_password": {
            "username": "testuser", "email": "test@example.com",
...
def test_list_users_as_country_admin_success(country_admin_client, country):
    ...
    User.objects.create_user(
        ...
        role=Role.COUNTRY_MEMBER, country=country
    )
    User.objects.create_user(
    ...
    other_country = Country.objects.create(name="Other Country", capital="Other Capital")
    User.objects.create_user(
        username="otheruser", email="other@test.com", password="TestPass123!",
        role=Role.COUNTRY_MEMBER, country=other_country
    )
...
@pytest.mark.django_db
def test_list_users_as_country_member_success(country_member_client, country_member_user, country):
    """Test country member can only see themselves"""
    # Create other users in same country
    User.objects.create_user(
        username="other1", email="other1@test.com", password="TestPass123!",
        role=Role.COUNTRY_MEMBER, country=country
    )
...
@pytest.mark.django_db
def test_create_user_as_country_admin_same_country_success(country_admin_client, country):
