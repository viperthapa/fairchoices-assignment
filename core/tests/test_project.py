...
from core.models import AuditLog, Project, ProjectStatus
...
def test_create_project_as_super_admin_success(super_admin_client, project_payload):
    ...
    assert AuditLog.objects.filter(
        model_name="Project", action="create", object_id=project.id
    ).exists()


@pytest.mark.django_db
def test_create_project_as_country_admin_success(
    country_admin_client, country, project_payload
):
...
def test_update_project_as_super_admin_success(super_admin_client, project):
    ...
    # Verify audit log was created
    assert AuditLog.objects.filter(
        model_name="Project", action="update", object_id=project.id
    ).exists()
...
def test_delete_project_as_super_admin_success(super_admin_client, project):
    ...
    assert AuditLog.objects.filter(
        model_name="Project", action="delete", object_id=project.id
    ).exists()


@pytest.mark.django_db
def test_delete_project_as_country_admin_same_country_success(
    country_admin_client, project
):
